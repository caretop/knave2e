 ...
 * @extends {ActorSheet}
 */
export class Knave2eCreatureSheet extends ActorSheet {

    /** @override */
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["knave2e", "sheet", "actor", "creature"],
            template: "systems/knave2e/templates/actor/creature-sheet.html",
            width: 680,
            height: 620,
    ...
    _prepareCharacterItems(sheetData) {
        ...
        for (let i of sheetData.items) {
            ...
            let pipHtml = "";
            for (let i = 0; i < item.coins.max; i++) {
                if (i < item.coins.value)
                    pipHtml += '<i class="fas fa-circle"></i>'
                else
                    pipHtml += '<i class="far fa-circle"></i>';
            }
            item.coins.html = pipHtml;
