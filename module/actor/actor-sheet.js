...
export class Knave2eActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["knave2e", "sheet", "actor", "character"],
  ...
  _prepareCharacterItems(sheetData) {
    ...
    for (let i of sheetData.items) {
      ...
      item.size.x = 17 + "em";
      item.size.y = (item.sheet.curHeight * 2.5) + "em";
  ...
  activateListeners(html) {
    ...
    //reach for item slot
    html.find('.item-slot-roll').click(ev => {
      const div = $(ev.currentTarget);
      //console.log(div[0].outerText)
      const slotTarget = div[0].outerText
      //console.log(slotTarget)
      this.actor.rollSlotReach(slotTarget) 
    });
    ...
    if (this.actor.isOwner) {
      ...
      //       yPos = Math.round(yPos / roundScale) * roundScale;// - (item.data.size.weight - 1) * 4;
      //     }
