 ...
 * @extends {ActorSheet}
 */
export class Knave2eStorageSheet extends ActorSheet {

    /** @override */
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["knave2e", "sheet", "actor", "storage"],
            template: "systems/knave2e/templates/actor/storage-sheet.html",
            width: 475,
            height: 500,
            resizable: false,
    ...
    getData() {
        ...
        data.data.data.size.divHeight = data.data.data.size.weight * 130 + 35;

        let storenum = 0;
        for (let y = 0; y < data.data.data.size.weight; y++) {
            for (let x = 0; x < data.data.data.size.width; x++) {
                storenum++;
                data.data.data.storeDiv += '\
                <div class="item-slot-dashed" style="transform: translate3d('+ (x * 130 - (data.data.data.size.width - 1) * 65) + 'px, ' + (y * 130 - (data.data.data.size.weight - 1) * 65) + 'px, 0px);">\
        ...
        this.position.height = data.data.data.size.weight * 130 + 230;
